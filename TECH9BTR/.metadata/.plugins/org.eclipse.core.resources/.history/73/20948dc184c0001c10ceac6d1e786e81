...
<body>
		<%  
			...
			int destpc =Integer.parseInt(request.getParameter("postal"));
			...
			try
			{
				...
				String command = "INSERT into BATCH9BTR_TRIP_DETAILS(user_id,trip_purpose_id,depart_date,return_date,depart_country,dest_addr,dest_city,dest_country,dest_postal_code,currency,est_cost,adv_amt,accomodation,submit_date) values('"+sid+"','"+tpurpose+"','"+deptdate+"','"+returndate+"','"+deptcountry+"','"+destaddr+"','"+destcity+"','"+destcountry+"','"+destpc+"','"+currency+"','"+estcost+"','"+advamt+"','"+accomod+"')";
